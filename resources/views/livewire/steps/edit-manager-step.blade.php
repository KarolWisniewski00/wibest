<div>
    ...
    <div>
        <div class="mb-4" id="manager">
            <h3 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">Przełożony</h3>
            <ul class="grid w-full gap-4 md:grid-cols-3">
                @foreach ($users as $user)
                @if ($user->role == 'admin' || $user->role == 'menedżer' || $user->role == 'właściciel')
                <li>
                    <input name="manager_id" wire:model="state.manager_id" type="radio" id="manager-{{ $user->id }}" value="{{ $user->id }}" class="hidden peer">
                    <label for="manager-{{ $user->id }}" class="h-full inline-flex items-center justify-between w-full p-4 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-green-300 dark:peer-checked:border-green-300 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
