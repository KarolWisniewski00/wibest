...
<tr class="snap-center bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 text-center">
    ...
    <td class="px-1 py-1 sticky md:left-0 md:z-20 bg-white dark:bg-gray-800">
        <div class="text-gray-600 dark:text-gray-300 flex justify-start items-center gap-1 font-semibold uppercase tracking-widest">
    ...
    @while ($i < $count)
        ...
        for ($j = $i + 1; $j < $count; $j++) {
            ...
            <td colspan="{{ $span }}"
                ...
                <a href="@if($role == 'admin' || $role == 'menedżer' || $role == 'właściciel') {{route('calendar.work-schedule.edit', $user->objs[$keys[$i]] )}} @endif" class=" flex flex-col items-center justify-center text-center 
            ...
            <td colspan="{{ $span }}"
                ...
                <a href="@if($user->role == 'admin' || $user->role == 'właściciel' || $user->role == 'menedżer') {{route('leave.pending.edit', $currentObj)}} @else {{route('leave.single.edit', $currentObj)}} @endif" class=" flex flex-col items-center justify-center text-center 
            ...
            <td colspan="{{ $span }}" title="{{ $startDate }} - {{ $endDate }}" class="px-1 py-1 font-semibold text-lg  text-gray-700 dark:text-gray-50 border-x border-gray-200 dark:border-gray-700">
            </td>
            @else
            <td colspan="{{ $span }}"
                title="{{ $startDate }} - {{ $endDate }}"
            ...
            @endwhile
            @else
            <td class="px-1 py-1 font-semibold text-lg  text-gray-700 dark:text-gray-50 border-x border-gray-200 dark:border-gray-700">
            </td>
            @endif
</tr>