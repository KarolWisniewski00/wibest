<div {{ $attributes->only(['class', 'wire:key'])->class('relative w-full') }}
    ...
    <div class="relative min-h-[46px]">
        ...
        <x-dynamic-component
            ...
            class="min-h-[46px] cursor-pointer overflow-hidden !text-transparent !dark:text-transparent selection:bg-transparent"
            x-ref="input"
            x-on:click="toggle"
            x-on:focus="open"
                ...
                <div :class="{
                    ...
                    <template x-if="!config.multiselect">
                        <div @class([
                            ...
                            <span
                                class="truncate text-secondary-700 dark:text-secondary-400 text-lg font-semibold"
                                x-show="!isEmpty()"
                        ...
                        <div class="absolute left-0 inset-y-0 pl-3 pr-14 w-full flex items-center overflow-hidden" x-on:click="toggle">
                            <div class="flex items-center gap-2 overflow-x-auto hide-scrollbar">
                                ...
                                @if (!$withoutItemsCount)
                                    <span
                                        class="inline-flex text-secondary-700 dark:text-secondary-400 text-lg font-semibold"
        ...
        @if ($hint)
            <label @if ($id) for="{{ $id }}" @endif class="mt-2 text-lg font-semibold text-secondary-500 dark:text-secondary-400">
                {{ $hint }}
            </label>
        @endif
    </div>
                ...
                <template x-for="(option, index) in displayOptions" :key="`${index}.${option.value}`">
                    <li tabindex="-1" :index="index">
                        <div class="px-2 py-0.5">
                            <div class="h-8 w-full animate-pulse bg-gray-200 dark:bg-gray-600 rounded"></div>
                        </div>
                    </li>
                </template>
            </ul>
