<x-app-layout class="flex">
    ...
    <x-main-no-filter>
        ...
        <x-container-content-form>
            ...
            <x-header-form>
                <span>🚀</span> Edycja Aktywności
            </x-header-form>
            <!--POWRÓT-->
            <form id="myForm" method="POST" action="{{ route('setting.crm.update', $crm) }}" class="space-y-4">
                @csrf
                @method('PUT')
                <style>
                ...
                <div class="mt-2">
                    ...
                    <select id="company" name="company" autocomplete="off" class="mt-2 rounded-lg text-lg">
                        ...
                        @foreach($companies as $company)
                            <option value="{{ $company['id'] }}" data-description="{{ $company['description'] }}" {{ $company['id'] == $crm->company_id ? 'selected' : '' }}>
                                🏢 {{ $company['name'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mt-2">
                    ...
                    <select id="user" name="user" autocomplete="off" class="mt-2 rounded-lg text-lg">
                        ...
                        @if($crm->user_id)
                            <option value="{{ $crm->user_id }}" data-role="{{ $crm->user->role }}" selected>
                                {{ $crm->user->name }}
                            </option>
                        @endif
                    </select>
                        ...
                        let userSelect = new TomSelect('#user', {
                            ...
                            render: {
                                option: function (data, escape) {
                                    ...
                                    return `
                                    <div class="px-3 py-2 dark:bg-gray-700 dark:text-white">
                                        <div class="flex flex-row gap-2 text-lg">s
                ...
                <div class="mt-2">
                    <div class="mt-2">
                        ...
                        <select id="type" name="type" multiple autocomplete="off" placeholder="Wybierz typ"
                            ...
                            <option value="spotkanie" data-description="Spotkanie z klientem" {{ $crm->type == 'spotkanie' ? 'selected' : '' }}>
                                👤 Spotkanie
                            </option>
                            <option value="telefon" data-description="Rozmowa telefoniczna z klientem" {{ $crm->type == 'telefon' ? 'selected' : '' }}>
                                📱 Telefon
                            </option>
                            <option value="mail" data-description="Wiadomość e-mail do klienta" {{ $crm->type == 'mail' ? 'selected' : '' }}>
                                📧 E-mail
                            </option>
                            <option value="oferta" data-description="Przesłanie oferty do klienta" {{ $crm->type == 'oferta' ? 'selected' : '' }}>
                                🛒 Oferta
                            </option>
                            <option value="follow_up" data-description="Follow up" {{ $crm->type == 'follow_up' ? 'selected' : '' }}>
                                🔄 Follow up
                            </option>
                            <option value="inne" data-description="Inne" {{ $crm->type == 'inne' ? 'selected' : '' }}>
                                📌 inne
                            </option>
                        </select>
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <input value="{{ old('datetime_start', $crm->datetime_start) }}" type="text" id="datetime_start"
                        name="datetime_start" placeholder="DD.MM.YYYY HH:MM" required
                ...
                <div class="mb-6">
                    <label for="datetime_end" class="block text-sm font-medium text-gray-700 dark:text-gray-300">📅 Data
                        zakończenia</label>
                    <input value="{{ old('datetime_end', $crm->datetime_end) }}" type="text" id="datetime_end"
                        name="datetime_end" placeholder="DD.MM.YYYY HH:MM" required
                        class="min-h-[46px] !text-lg mt-1 block w-full p-2 border border-gray-300 rounded-md shadow focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100">
                    @error('datetime_end')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mt-2 flex flex-col">
                    ...
                    <label class="inline-flex items-center cursor-pointer mt-2">
                        <input type="checkbox" {{ old('already_realized', $crm->datetime_complete) ? 'checked' : '' }}
                            name="already_realized" class="sr-only peer">
                    ...
                    <!-- Resetuj urlopy planowane -->
                    <label class="inline-flex items-center cursor-pointer mt-2">
                        <input type="checkbox" {{ old('important', $crm->important) ? 'checked' : '' }} name="important"
                            class="sr-only peer">
                        <div class="relative w-11 h-6 bg-gray-200 rounded-full peer dark:bg-gray-700
                        peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full
                        peer-checked:after:border-white
