                ...
                @if($role == 'admin' || $role == 'menedżer' || $role == 'właściciel')
                    @if($work_session->status != 'Praca zakończona')
                        <div class="p-2 text-sm text-yellow-300 rounded-lg dark:text-yellow-300" role="alert">
                            <span>⚠️ Ostrzeżenie!</span> Edycja nie jest możliwa, ponieważ sesja pracy jeszcze nie została
                            zakończona.
                        </div>
                    @endif
                @endif
            ...
            <x-container-content-form>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-4">
                    <x-container-gray>
                        ...
                        <x-text-cell>
                            ...
                            <x-text-cell-value>
                                <x-text-cell-span>
                                    <span>
                                        ⏱️ <x-RCP.work-session-status :work_session="$work_session"
                                            class="text-2xl m-0 p-0" />
                                    </span>
                                </x-text-cell-span>
                            </x-text-cell-value>
                        </x-text-cell>
                                        ...
                                        @else
                                            ...
                                            <div class="flex flex-col items-center justify-center gap-2 my-auto">
                                                <x-paragraf-display
                                                    class="text-2xl whitespace-nowrap font-semibold w-fit text-start relative">
                                                    <x-status-dark>
                                                        {{\Carbon\Carbon::parse(\App\Models\Event::where('id', $work_session->event_start_id)->first()->time)->format('H:i')}}
                                                        - TERAZ
                                                    </x-status-dark>
                                                </x-paragraf-display>
                                                <x-paragraf-display id="counting_work_session"
                                                    class="text-gray-900 dark:text-gray-50 text-2xl whitespace-nowrap font-semibold w-fit text-start relative">
                                                    00:00:00
                                                </x-paragraf-display>
                                            </div>
                                            ...
                                            @endphp
                                            <input type="hidden" id="seconds_elapsed" name="seconds_elapsed"
                                        ...
                                        <x-text-cell-value>
                                            <x-text-cell-span class="gap-2 w-full">
                                                <div class="flex flex-col items-start w-full justify-start gap-2">
                                                    <x-button-link-neutral class="text-lg w-full"
                                                        href="{{ route('rcp.work-session.fix', $work_session) }}">
                            ...
                            @if($work_session->eventStop == null && $work_session->status == 'W trakcie pracy')
                                <x-text-cell>
                                    ...
                                    <x-text-cell-value>
                                        <x-text-cell-span class="gap-2 w-full">
                                            <div class="flex flex-col items-start w-full justify-start gap-2">
                                                <x-button-link-red class="text-lg w-full"
                                                    href="{{ route('rcp.work-session.stop', $work_session) }}">
                                    ...
                                    @if($work_session->status == 'Praca zakończona' && $work_session->time_in_work == '24:00:00' && $work_session->user->working_hours_from && $work_session->user->working_hours_to && $work_session->user->working_hours_start_day && $work_session->user->working_hours_stop_day)
                                        <x-text-cell>
                                            ...
                                            <x-text-cell-value>
                                                <x-text-cell-span class="gap-2 w-full">
                                                    <div class="flex flex-col items-start w-full justify-start gap-2">
                                                        <x-button-link-green class="text-lg w-full"
                                                            href="{{ route('rcp.work-session.start.plus', $work_session) }}">
                                                            <i class="fa-solid fa-clock mr-2"></i>start +
                                                            {{$work_session->user->working_hours_custom}}h
                                                        </x-button-link-green>
                                                        <x-button-link-red class="text-lg w-full"
                                                            href="{{ route('rcp.work-session.stop.minus', $work_session) }}">
                                                            <i class="fa-solid fa-clock mr-2"></i>stop -
                                                            {{$work_session->user->working_hours_custom}}h
                                                        </x-button-link-red>
                                                    </div>
                                                </x-text-cell-span>
                            ...
                            <x-text-cell>
                                ...
                                <x-text-cell-value>
                                    <x-text-cell-span class="gap-2 w-full">
                                        <div class="flex flex-col items-start w-full justify-start gap-2">
                                            <x-button-link-red class="text-lg w-full"
                                                href="{{ route('rcp.work-session.start.plus.fix', $work_session) }}">
                    ...
                    <x-container-gray>
                        ...
                        @endif
                        @if($work_session->notes == null)
                            <x-button-link-green href="{{ route('rcp.work-session.create.note', $work_session) }}"
                                class="text-lg">
                    ...
                    @if($work_session->eventStart != null)
                        ...
                        <x-container-gray>
                            ...
                            <input id="start-time-picker" type="datetime-local" value="{{ $startTime }}"
                                max="{{ now()->format('Y-m-d\ H:i') }}" class="hidden" />
                            <x-text-cell>
                                <x-text-cell-label>
                                    Przesuń rozpoczęcie pracy
                                </x-text-cell-label>
                                <x-text-cell-value>
                                    <x-text-cell-span class="gap-2 w-full" id="start-time-wrapper">
                                        <div class="flex flex-col items-start w-full justify-start gap-2">
                                            <x-button-blue href="" id="open-start-time" class="text-lg w-full">
                                                <i class="fa-solid fa-pen-to-square mr-2"></i>Edycja startu
                                            </x-button-blue>
                                        </div>
                                    </x-text-cell-span>
                                </x-text-cell-value>
                            ...
                            <script>
                                ...
                                $(document).ready(function () {
                                    ...
                                    const $wrapper = $('#start-time-wrapper');
                                    const $input = $('#start-time-picker');

                                    let originalValue = $input.val();

                                    // 🔵 wejście w tryb edycji
                                    $(document).on('click', '#open-start-time', function (e) {
                                        e.preventDefault();

                                        originalValue = $input.val();
                                        const time = originalValue.split('T')[1];
                                        const now = new Date();
                                        const nowTime = now.toTimeString().split(' ')[0]; // HH:mm:ss
                                        const inputHtml = `
                                        <form method="POST" action="{{ route('rcp.work-session.update.start', $work_session) }}" class="flex flex-col w-full">
                                            @method('PUT')
                                            @csrf
                                            ...
                                            <div class="relative mb-4 border-gray-300">
                                                <input
                                                    type="time"
                                                    step="1"
                                                    id="editing-start-time"
                                                    name="editing_start_time"
                                                    value="${time}"
                                                    max="${nowTime}"
                                            ...
                                            <div class="flex flex-col gap-2">
                                                <x-button-green type="submit" class="text-lg w-full">
                                                    <i class="fa-solid fa-floppy-disk mr-2"></i>Zapisz
                                                </x-button-green>
                                                <x-button-neutral type="button" id="cancel-start-time" class="text-lg w-full">
                                                    <i class="fa-solid fa-chevron-left mr-2"></i>Anuluj
                                                </x-button-neutral>
                                            </div>
                                        </form>
                                                            `;
                                    ...
                                    // 🔴 cancel
                                    $(document).on('click', '#cancel-start-time', function () {
                                        restoreButton();
                                    });

                                    $(document).on('change input', '#editing-start-time', function () {
                                        ...
                                        // 👉 budujemy pełną datę (dzisiaj + czas)
                                        ...
                                        if (selected > current) {
                                            toastr.error('Nie możesz wybrać przyszłej godziny');
                                            $(this).val('');
                                            return;
                                        }
                                    ...
                                    function restoreButton(newValue = null) {
                                        if (newValue) {
                                            $input.val(newValue);
                                        }
                            ...
                            @if($work_session->eventStart->location)
                                ...
                                <input type="hidden" id="longitude_start"
                                    value="{{$work_session->eventStart->location->longitude}}">
                                <script>
                                    $(document).ready(function () {
                ...
                <div class="flex flex-col md:flex-row justify-end gap-4 mt-4">
                    @if($role == 'admin' || $role == 'menedżer' || $role == 'właściciel')
                        @if($work_session->status == 'Praca zakończona')
                            ...
                            <!--EDYTUJ-->
                        @endif
                        <form action="{{route('rcp.work-session.delete', $work_session)}}" class="w-full md:w-fit" method="POST"
                ...
                <x-label class="py-2">
                    Utworzono
                    {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $work_session->created_at)->format('d.m.Y H:i:s') ?? '' }}
                </x-label>
                <x-label class="py-2">
                ...
                <x-label class="py-2">
                    ...
                    {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $work_session->updated_at)->format('d.m.Y H:i:s') ?? '' }}
                </x-label>
                <x-label class="py-2">
                    Historia zmian
                </x-label>
                    ...
                    <div class="max-w-2xl flex flex-col gap-4">
                        @foreach($logs as $log)
                            @php
                                ...
                                $source = class_basename($log->subject_type); // WorkSession, Event
                                if ($source === 'Event') {
                                    if ($log->subject_id == $work_session->task_id) {
                                        $source .= ' (task)';
                                    } else {
                                        $source .= $log->subject_id == $work_session->event_start_id ? ' (start)' : ' (stop)';
                                    }
                                }
                            @endphp
                                    ...
                                    @foreach($new as $field => $value)
                                        <div>
                                            <span class="font-semibold">
                                                @if($source == 'Event (start)')
                                                    <x-status-green>
                                                        🟢 Start
                                                    </x-status-green>
                                                @elseif($source == 'Event (stop)')
                                                    <x-status-red>
                                                        🔴 Stop
                                                    </x-status-red>
                                                @elseif($source == 'Event (task)')
                                                    <x-status-gray>
                                                        🎯 Zadanie
                                                    </x-status-gray>
                                                @endif
                                            ...
                                            <span class="text-red-300 line-through mr-2">{{ $old[$field] ?? '-' }}</span>
