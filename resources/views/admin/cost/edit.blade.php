<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8 bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <!--FORMULARZ-->
                    <div class="mt-8">
                        <h1 class="text-2xl font-medium text-gray-900 dark:text-gray-100">Edycja Faktury Kosztowej</h1>
                        <form method="POST" action="{{ route('cost.update', $cost->id) }}" enctype="multipart/form-data" class="mt-6">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-6">
                                ...
                                <input type="text" id="number" name="number" value="{{ old('number', $cost->number) }}" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100">
                                @error('number')
                            ...
                            <div class="mb-6">
                                ...
                                <input type="number" step="0.01" id="total" name="total" value="{{ old('total', $cost->total) }}" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100">
                                @error('total')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <input type="date" id="due_date" name="due_date" value="{{ old('due_date', $cost->due_date) }}" required class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100">
                                @error('due_date')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea id="notes" name="notes" rows="4" class="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100">{{ old('notes', $cost->notes) }}</textarea>
                                @error('notes')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="w-full relative border-2 border-gray-300 border-dashed rounded-lg p-6" id="dropzone">
                                    ...
                                    <img src="{{ asset('attachments/'.$cost->path) }}" class="mt-4 mx-auto max-h-40" id="preview" alt="Preview of uploaded file">
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Ukrycie podglądu i opisu, jeśli plik już istnieje
            if ($('#preview').attr('src')) {
                $('#upload-description').addClass('hidden');
            }
        });
