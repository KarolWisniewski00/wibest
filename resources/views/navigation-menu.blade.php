<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 dark:border-gray-700 shadow fixed w-full"
        ...
        <div class="flex justify-between h-20">
            <div class="flex">
                ...
                <div class="hidden space-x-8 xl:-my-px xl:ms-4 xl:flex">
                    ...
                    @endif
                    @if($role == 'admin' || $role == 'właściciel')
                    <x-nav-link href="{{ route('calendar.work-schedule.index') }}"
                        :active="Str::startsWith(request()->path(), 'dashboard/calendar')">
                        <i class="fa-solid fa-calendar-days mr-2"></i>Grafik
                    </x-nav-link>
                    @else
                    <x-nav-link href="{{ route('calendar.work-schedule.calendar.user') }}"
                        :active="Str::startsWith(request()->path(), 'dashboard/calendar')">
                        <i class="fa-solid fa-calendar-days mr-2"></i>Grafik
                    </x-nav-link>
                    @endif
                    <x-nav-link href="{{ route('leave.single.index') }}" :active="Str::startsWith(request()->path(), 'dashboard/leave')">
                        <i class="fa-solid fa-inbox mr-2"></i>E-Wnioski
                    </x-nav-link>
                    @if($role == 'admin' || $role == 'właściciel')
                    ...
                    <x-nav-link href="{{ route('raport.time-sheet.index') }}"
                        :active="Str::startsWith(request()->path(), 'dashboard/raport')">
                        <i class="fa-solid fa-chart-line mr-2"></i>Raporty
                    </x-nav-link>
                    @else
                    <x-nav-link href="{{ route('rcp.work-session.index') }}" :active="Str::startsWith(request()->path(), 'dashboard/rcp')">
                        <i class="fa-solid fa-clock mr-2"></i>Czas pracy
                    </x-nav-link>
                    @endif
                    @if($role == 'admin' || $role == 'właściciel')
                                    ...
                                    <x-paragraf-display class="text-xs font-bold text-gray-900 dark:text-white">
                                        <div class="flex flex-row gap-1 justify-start items-center">
                                            ...
                                            <div class="flex flex-row gap-1 items-center">
                                                <div
                                                    class="inline-flex items-center text-gray-600 dark:text-gray-300 font-semibold uppercase tracking-widest hover:text-gray-700 dark:hover:text-gray-300 transition ease-in-out duration-150 text-md">
                                                    Święto ustawowo wolne</div>
                                                <span
                                                    class="px-3 py-1 mx-2 rounded-full text-[0.5rem] w-fit font-semibold bg-rose-300 text-gray-900 font-semibold uppercase tracking-widest hover:bg-rose-200 dark:hover:bg-rose-400 focus:bg-rose-200 dark:focus:bg-rose-300 active:bg-rose-200 dark:active:bg-rose-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                                                    ŚUW
                                                </span>
                                            </div>
                                        </div>
                        ...
                        <x-widget-display-nav class="grid-cols-3 grid-rows-1 gap-4 p-2 m-2 w-full">
                            <div class="col-span-2">
                                ...
                                <div class="">
                                    <x-flex-center>
                                        <div id="loaderDate"
                                            class="h-4 rounded-lg bg-gray-200 dark:bg-gray-600 w-16 mx-auto animate-pulse">
                                        </div>
                                ...
                                <div>
                                    <x-flex-center>
                                        <div id="loaderClock"
                                            class="h-4 mt-2 rounded-lg bg-gray-200 dark:bg-gray-600 w-12 mx-auto animate-pulse">
                                        </div>
                                        <x-paragraf-display id="timer" class="hidden text-lg">
                                            00:00:00
                                        </x-paragraf-display>
                                    </x-flex-center>
                                </div>
                    ...
                    <x-dropdown-jets align="right" width="48">
                        ...
                        <x-slot name="content">
                            ...
                            <x-dropdown-link href="{{ route('profile.show') }}">
                                <i class="fa-solid fa-user-gear mr-2"></i>Ustawienia konta
                            </x-dropdown-link>

                            <x-dropdown-link href="{{ route('team.user.config_sms', Auth::user()->id) }}">
                                <i class="fa-solid fa-comment-sms mr-2"></i>Ustawienia SMS
                            </x-dropdown-link>
                            ...
                            <form method="POST" action="{{ route('logout') }}" x-data>
                                ...
                                <x-dropdown-link href="{{ route('logout') }}" @click.prevent="$root.submit();">
                                    <i class="fa-solid fa-right-from-bracket mr-2"></i>Wyloguj
                                </x-dropdown-link>
                            </form>
                        </x-slot>
                    </x-dropdown-jets>
            ...
            <div class="me-4 flex items-center xl:hidden">
                <button @click="open = ! open" data-collapse-toggle-menu
                    class="inline-flex items-center justify-center p-4 rounded-md text-gray-400 dark:text-gray-500 hover:text-gray-500 dark:hover:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900 focus:outline-none focus:bg-gray-100 dark:focus:bg-gray-900 focus:text-gray-500 dark:focus:text-gray-400 transition duration-150 ease-in-out">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden xl:hidden">
        <div class="pt-2 pb-2 space-y-1">
            ...
            @endif
            @if($role == 'admin' || $role == 'właściciel')
            <x-responsive-nav-link href="{{ route('calendar.work-schedule.index') }}"
                :active="Str::startsWith(request()->path(), 'dashboard/calendar')">
                <i class="fa-solid fa-calendar-days mr-2"></i>Grafik
            </x-responsive-nav-link>
            @else
            <x-responsive-nav-link href="{{ route('calendar.work-schedule.calendar.user') }}"
                :active="Str::startsWith(request()->path(), 'dashboard/calendar')">
                <i class="fa-solid fa-calendar-days mr-2"></i>Grafik
            </x-responsive-nav-link>
            @endif
            ...
            @if($role == 'admin' || $role == 'właściciel')
            <x-responsive-nav-link href="{{ route('rcp.work-session.index') }}"
                :active="Str::startsWith(request()->path(), 'dashboard/rcp')">
                <i class="fa-solid fa-clock mr-2"></i>RCP
            </x-responsive-nav-link>
            <x-responsive-nav-link href="{{ route('raport.time-sheet.index') }}"
                :active="Str::startsWith(request()->path(), 'dashboard/raport')">
                <i class="fa-solid fa-chart-line mr-2"></i>Raporty
            </x-responsive-nav-link>
            @else
        ...
        <div class="py-2 pb-1 border-t border-gray-200 dark:border-gray-600">
            ...
            <div class="space-y-1">
                ...
                <x-responsive-nav-link href="{{ route('team.user.config_sms', Auth::user()->id) }}"
                    :active="Str::startsWith(request()->path(), 'dashboard/team/user/config_sms')">
                    <i class="fa-solid fa-comment-sms mr-2"></i>Ustawienia SMS
                </x-responsive-nav-link>
